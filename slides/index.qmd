...
- __Chaque _base learner_ essaie de corriger les erreurs du modèle issu des étapes précédentes.__


## Modélisation additive par étapes {.small90}

[__La modélisation additive par étapes simplifie massivement l'entraînement du modèle__]{.orange}:
...
[__À chaque étape, le _base learner_ essaie de corriger les erreurs du modèle issu des étapes précédentes.__]{.orange}
...
Hypothèses: on veut entraîner le modèle $F\left(\mathbf{x}\right) = \sum_{m=1}^M \beta_m f\left(\mathbf{x}, \mathbf{\theta}_m\right)$ avec une fonction de perte quadratique $L\left(y, F\left(\mathbf{x}\right)\right) = \left(y - F\left(\mathbf{x}\right)\right)^2$.
...
- [Le $m$-ième modèle essaie de prédire les résidus, c'est-à-dire ce que les $m-1$ arbres précédents n'ont pas réussi à prédire!]{.orange}
...
- Chaque arbre n'est pas indépendant des arbres précédents;
- Chaque arbre est ciblé sur les erreurs laissées par les arbres précédents $\Rightarrow$ cela rend le modèle final performant;
- Cette focalisation sur les erreurs crée un risque de surapprentissage de l'ensemble d'entraînement.

## Résumé sur le _boosting_
