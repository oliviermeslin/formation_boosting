...
L'objectif général des algorithmes d'apprentissage supervisé (_supervised learning_) est d'[__approximer une fonction__]{.orange} $F$ qui relie une variable $y$ à un ensemble de variables $\mathbf{x}$. On obtient une fonction $\hat{F}$:
...
La [__fonction de perte__]{.orange} $L$ mesure l'écart entre l'observé $y$ et la prédiction $F\left(\mathbf{x} \right)$.

. . .

Fonctions de perte classiques:

- La perte quadratique: [$L\left(y, F\left(\mathbf{x}\right)\right) =  \left(y - F\left(\mathbf{x}\right)\right)^2$]{.small80};
- La perte absolue: [$L\left(y, F\left(\mathbf{x}\right)\right) =  \left|y - F\left(\mathbf{x}\right)\right|$]{.small80};
- La perte logloss: [$L\left(y, F\left(\mathbf{x}\right)\right) = - \left(y \log \left(p\left(\mathbf{x}\right)\right) + \left(1 - y \right) \log \left(1 - p\left(\mathbf{x}\right)\right)\right)$]{.small80}.
...
À partir d'un jeu de données $\left(y_i, \mathbf{x_i} \right)_{i= 1,\dots,n}$, on obtient l'approximation $\hat{F}$ en minimisant l'espérance conditionnelle de la fonction de perte $L$:
