...
- On écrit la fonction de perte au moment de l'entraînement du $t$-ième arbre: $$\begin{aligned}\mathcal{L}^{(t)} 
...
- On définit $I_j = \{ i | q(\mathbf{x}_i) = j \}$ l'ensemble des observations situées sur la feuille terminale $j$ et on réorganise l'expression de $\mathcal{\tilde{L}}^{(t)}$ pour faire apparaître les poids $w_i$:
$$\begin{aligned}
\mathcal{\tilde{L}}^{(t)}
&= \sum_{j=1}^{T} \sum_{i\in I_j} [g_i f_t(\mathbf{x}_i) &+ \frac{1}{2} h_i f^2_t(\mathbf{x}_i)] &+ \gamma T + \frac{1}{2} \lambda \sum_{j=1}^T w_i^2 \\
&= \sum_{i=1}^{n} l(y_i, \hat{y}_{i}^{(t-1)} &+ f_{t}(\mathbf{x}_i)) &+ \Omega(f_t) + constant
\end{aligned}$$
...
\mathcal{\tilde{L}}^{(t)}
  &= \sum_{j=1}^{T} \sum_{i\in I_j} [g_i f_t(\mathbf{x}_i) &+ \frac{1}{2} h_i f^2_t(\mathbf{x}_i)] &+ \gamma T + \frac{1}{2} \lambda \sum_{j=1}^T w_i^2 \\
  &= \sum_{j=1}^{T} \sum_{i\in I_j} [g_i w_i &+ \frac{1}{2} h_i w_i^2] + \gamma T &+ \frac{1}{2} \lambda \sum_{j=1}^T w_i^2 \\
