...
- Chaque arbre est défini par trois [paramètres]{.orange}: sa structure (fonction $q: \mathbb{R}^m \rightarrow T$), son nombre de feuilles terminales $T$ et ses poids $\mathbf{w}\in \mathbb{R}^T$.
...
On écrit la fonction de perte au moment de l'entraînement du $t$ième arbre:

$$\begin{aligned}\mathcal{L}^{(t)} 
...
On fait un développement limité d'ordre 2 de $l(y_i, \hat{y}_{i}^{(t-1)} + f_{t}(\mathbf{x}_i))$ au voisinage de $f_{t}(\mathbf{x}_i)$:
