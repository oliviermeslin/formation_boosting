...
# La _gradient boosting machine_

## Estimation d'une fonction inconnue

L'objectif général des algorithmes d'apprentissage supervisé (_supervised learning_) est d'[__approximer une fonction inconnue__]{.orange} $F: \mathbf{x} \mapsto y$ en minimisant l'espérance conditionnelle d'une fonction de perte $L\left(y, F\left(\mathbf{x}\right)\right)$ sur un ensemble d'entraînement $\left(y_i, \mathbf{x_i} \right)_{i= 1,\dots,n}$. Formellement:
...
Une restriction fréquente consiste à rechercher $F\left(\mathbf{x}\right)$ dans un ensemble de fonctions paramétrisées par un vecteur fini de paramètres $\theta$. <br/>
Exemple: $F\left(\mathbf{x}\right) = \mathbf{x\beta}$ avec $\mathbf{\beta} = \{\beta_1, \dots, \beta_d\}$

. . .

Cette restriction permet de se ramener à un problème d'optimisation sur l'espace des paramètres: $\hat{F}$ est caractérisée par ses paramètres $\hat{\bf\beta}$ tel que
