...
Une approche d'apprentissage supervisé comprend toujours trois éléments:

::: {.small90}

- Un [__modèle__]{.orange} $F$ qui relie $y$ et $\mathbf{x}$ et $y = F\left(\mathbf{x}, \mathbf{\Theta}\right)$;<br/> 
Exemple: $y = \mathbf{x} \beta$

- Des [__paramètres__]{.orange} $\Theta = \{w_j\}_{j=1,\dots,d}$ qui doivent être estimés à partir des données; <br/> 
Exemple: $\beta$ dans $y = \mathbf{x} \beta$
...
Petits abus de notation: 
- on écrit $F\left(\mathbf{x}\right)$ au lieu de $F\left(\mathbf{x}, \mathbf{\Theta}\right)$;
- on écrit $Obj\left(\Theta\right)$ au lieu de $Obj\left(\Theta, y, \mathbf{x}\right)$;
...
$$Obj\left(\Theta, y, \mathbf{x}\right) = \underbrace{L\left(\Theta, y, \mathbf{x}\right)}_{\substack{\bf\text{Fonction} \\ \bf\text{de perte}}} + \underbrace{\Omega\left(\Theta\right)}_{\substack{\bf\text{Fonction de} \\ \bf\text{régularisation}}}$$
...
- La [__fonction de régularisation__]{.orange} $\Omega$ mesure la complexité du modèle. Elle croît avec cette complexité:
  - Nombre de paramètres;
  - Valeur absolue des paramètres.
...
À partir d'un jeu de données $\left(y_i, \mathbf{x_i} \right)_{i= 1,\dots,n}$, on obtient l'approximation $\hat{F}$ en [minimisant l'espérance conditionnelle]{.orange} de la fonction bjectif $Obj$.

$$\hat{F} = \argmin_F \mathbb{E}_{y,\mathbf{x}}\left[ Obj\left(y, F\left(\mathbf{x}\right)\right)\right] $$

. . .

Exemple: On modélise une variable continue avec:

- un modèle linéaire: $F\left(\mathbf{x}\right) = \mathbf{x} \beta$; 
...
|| Données utilisées pour l'entraînement du modèle | _Training set_ |  |
|| Données utilisées pour l'évaluation du modèle | _Test set_ |  |
