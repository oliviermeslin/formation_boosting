...
- Un [__modèle__]{.orange} $F$ qui relie $y$ et $\mathbf{x}$ et $y = F\left(\mathbf{x}\right)$;<br/> 
...
- Une [__fonction objectif__]{.orange} $Obj(\Theta)$ avec une [fonction de perte]{.orange} et un [terme de régularisation]{.orange}.<br/>
:::
...
$$Obj\left(\Theta\right) = \underbrace{L\left(\Theta\right)}_{\substack{\bf\text{Fonction} \\ \bf\text{de perte}}} + \underbrace{\Omega\left(\Theta\right)}_{\substack{\bf\text{Fonction de} \\ \bf\text{régularisation}}}$$
...
- Cette fonction est habituellement convexe et dérivable deux fois.

. . .

- Fonctions de perte classiques:
  - La perte quadratique: [$L\left(y, F\left(\mathbf{x}\right)\right) =  \left(y - F\left(\mathbf{x}\right)\right)^2$]{.small90}
  - La perte absolue: [$L\left(y, F\left(\mathbf{x}\right)\right) =  \left|y - F\left(\mathbf{x}\right)\right|$]{.small90}
  - La perte logloss: [$L\left(y, F\left(\mathbf{x}\right)\right) = - \left(y \log \left(p\left(\mathbf{x}\right)\right) + \left(1 - y \right) \log \left(1 - p\left(\mathbf{x}\right)\right)\right)$]{.small90}
...
À partir d'un jeu de données $\left(y_i, \mathbf{x_i} \right)_{i= 1,\dots,n}$, on obtient l'approximation $\hat{F}$ en [minimisant l'espérance conditionnelle]{.orange} de la fonction de perte $L$.
...
- un modèle linéaire: [$F\left(\mathbf{x}\right) = \mathbf{x} \beta$]{.small90}; 
- une fonction de perte quadratique: [$L\left(y, F\left(\mathbf{x}\right)\right) =  \left(y - F\left(\mathbf{x}\right)\right)^2$]{.small90}. 
