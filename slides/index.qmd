...
- Principe: créer un modèle très précis (_strong learner_) en combinant un grand nombre de modèles simples (_base learners_ ou _weak learners_).
...
- Exemple: $$F\left(\mathbf{x}\right) = \sum_{m=1}^M \beta_m f\left(\mathbf{x}, \mathbf{\theta}_m\right)$$
...
- Trois grandes approches ensemblistes:
  ...
  - _Bagging_ (@breiman1996bagging): chaque modèle est entraîné séparément sur un échantillon aléatoire d'observations;
  - _Random forest_ (@breiman2001random): chaque modèle est entraîné séparément sur un échantillon aléatoire d'observations et de variables;
  - _Boosting_ (@shapire1990strength): chaque modèle essaie de corriger les modèle erreurs du précédent.
