...
Dans une approche de _boosting_, on veut entraîner le modèle suivant:
...
- L'entraînement commence par entraîner un seul _base learner_;
- On ajoute ensuite séquentiellement à ce premier modèle d'autres _base learners_, sans modifier les paramètres des modèles déjà entraînés;
- À chaque itération, l'entraînement essaie de trouver le _base learner_ qui améliore le plus possible le modèle global.

. . .

__On décompose l'entraînement d'un très gros modèle (procédure très complexe) en une série d'entraînements de petits modèles (procédure beaucoup plus simple).__
