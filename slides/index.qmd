...
- Principe: rechercher $F\left(\mathbf{x}\right)$ dans un ensemble de fonctions paramétrisées par un vecteur fini de paramètres $\theta$. <br/>
[Exemple: $F\left(\mathbf{x}\right) = \mathbf{x\beta}$ avec $\mathbf{\beta} = \{\beta_1, \dots, \beta_d\}$]{.small90}

. . .

- Cette restriction permet de se ramener à un problème d'optimisation sur l'espace des paramètres (plus simple à résoudre): $\hat{F}$ est caractérisée par ses paramètres $\hat{\bf\theta}$ tels que
...
- Les méthodes ensemblistes sont diversifiées:
  ...
  - La nature du _base learner_;
  - La méthode pour combiner les _base learners_ (moyenne, vote majoritaire...);
  - La méthode d'entraînement du modèle.

. . .

- Ces approches reposent sur un __fondement théorique précis__ (@shapire1990strength).
...
- Trois grandes approches ensemblistes:
  ...
  - _Bagging_ (@breiman1996bagging): chaque modèle est entraîné sur un échantillon aléatoire d'observations;
  - _Random forest_ (@breiman2001random): chaque modèle est entraîné sur un échantillon aléatoire d'observations et de variables;
  - _Boosting_: chaque modèle essaie de corriger les modèle erreurs du précédent.
...
Les approches de _boosting_ sont complètement différentes, et combinent deux caractéristiques:

- Une [__approche ensembliste__]{.orange} (_ensemble method_);
- Une [__modélisation additive linéaire__]{.orange} (_forward stagewise additive modeling_).
...
Modélisation additive linéaire (_forward stagewise additive modeling_)
